...
export default function Settings({ isAdmin }: { isAdmin: boolean }) {
  ...
  async function restoreBackupFile(file: File) {
    ...
    try {
      ...
      setBackupMsg(`Restored ${d.restored} file(s). Reloading…`);
  ...
  return (
    <main className="mx-auto max-w-2xl px-4 py-6">
      ...
      <div className="space-y-4">
        ...
        {/* Backup & Restore */}
        <Section icon={<DatabaseBackup size={16} />} title="Backup &amp; restore">
          <p className="mb-3 text-sm text-slate-500 dark:text-slate-400">
            Download a full backup (<code>.cnbdu</code>) of your settings, devices, automations, routines and groups.
            ⚠️ It contains password hashes and keys — keep the file safe. Restoring overwrites everything and may sign
            everyone out.
          </p>
          <div className="flex flex-wrap items-center gap-2">
            ...
            <label className="btn-ghost inline-flex cursor-pointer">
              {restoring ? <Loader2 size={15} className="animate-spin" /> : <Upload size={15} />} Restore…
              <input
                type="file"
                accept=".cnbdu,application/octet-stream"
                className="hidden"
                onChange={(e) => {
                  const f = e.target.files?.[0];
          ...
          {backupMsg && <p className="mt-2 text-sm text-slate-600 dark:text-slate-300">{backupMsg}</p>}
        </Section>
